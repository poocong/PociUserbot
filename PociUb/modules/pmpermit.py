...
from PociUb import TEMP_SETTINGS
from PociUb.helpers.adminHelpers import DEVS
from PociUb.helpers.basic import edit_or_reply
from PociUb.helpers.SQL.globals import addgvar, gvarstatus
from PociUb.helpers.tools import get_arg

from .help import add_command_help

DEF_UNAPPROVED_MSG = (
    "**[ BOT ASSISTEN POCI-UB ]**\n"
    "━━━━━━━━━━━━━━━━━━\n"
    "__HALLO SELAMAT DATANG, SAYA ADALAH BOT ASSISTEN, MOHON JANGAN MELAKUKAN SPAM, KARNA SAYA OTOMATIS AKAN MEMBLOKIR ANDA, TUNGGU SAMPAI SAYA MENERIMA PESAN ANDA__\n"
    "┏━━━━━━━━━━━━━━━━━\n"
    "┣[• PESAN OTOMATIS\n"
    "┣[• BY POCI USERBOT\n"
    "┗━━━━━━━━━━━━━━━━━"
...
async def incomingpm(client: Client, message: Message):
    try:
        ...
        from PociUb.helpers.SQL.pm_permit_sql import is_approved
...
async def auto_accept(client, message):
    try:
        from PociUb.helpers.SQL.pm_permit_sql import approve, is_approved
    except BaseException:
        pass
    ...
    if message.chat.id in DEVS:
        try:
            ...
            await client.send_message(
                message.chat.id,
                f"<b>Menerima Pesan!!!</b>\n{message.from_user.mention} <b>Terdeteksi Developer Userbot</b>",
...
async def approvepm(client: Client, message: Message):
    try:
        from PociUb.helpers.SQL.pm_permit_sql import approve
    except BaseException:
...
async def disapprovepm(client: Client, message: Message):
    try:
        from PociUb.helpers.SQL.pm_permit_sql import dissprove
    except BaseException:
        await message.edit("Running on Non-SQL mode!")
        return
...
async def setpm_limit(client: Client, cust_msg: Message):
    ...
    try:
        from PociUb.helpers.SQL.globals import addgvar
...
@Client.on_message(filters.command(["pmpermit", "pmmsg"], cmd) & filters.me)
...
@Client.on_message(filters.command("getpmmsg", cmd) & filters.me)
...
@Client.on_message(filters.command("resetpmmsg", cmd) & filters.me)
