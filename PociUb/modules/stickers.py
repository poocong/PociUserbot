...
from PociUb.helpers.tools import get_arg, get_text, resize_media
from PociUb.utils.tools import add_text_img, bash
...
async def kang(client: Client, message: Message):
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            return
        media_ = await client.download_media(replied, file_name="PociUb/resources/")
    else:
...
async def tinying(client: Client, message: Message):
    ...
    im1 = Image.open("PociUb/resources/blank.png")
    if ik.endswith(".tgs"):
