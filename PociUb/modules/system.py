...
from PociUb import BOTLOG_CHATID, LOGGER
from PociUb.helpers.basic import edit_or_reply
from PociUb.helpers.misc import HAPP
...
async def shutdown_bot(client: Client, message: Message):
    ...
    await edit_or_reply(message, "**Userbot Berhasil di matikan!**")
    if HAPP is not None:
...
async def logs_ubot(client: Client, message: Message):
    ...
    await client.send_document(
        message.chat.id,
        "Logs-Heroku.txt",
        thumb="PociUb/resources/logo.jpg",
