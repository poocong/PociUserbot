...
from PociUb.helpers.misc import HAPP, XCB
from PociUb.helpers.tools import get_arg
from PociUb.utils.misc import restart
from PociUb.utils.pastebin import PasteBin
from PociUb.utils.tools import bash
...
async def upstream(client: Client, message: Message):
    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await status.edit(
            "`[HEROKU]: Update Deploy Userbot Sedang Dalam Proses...`"
        )
        ups_rem.fetch(ac_br)
        ...
        try:
            ...
            pass
        await status.edit(
            "`Userbot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`"
        )
    else:
        try:
            ups_rem.pull(ac_br)
...
add_command_help(
    "update",
    [
        ["update", "Untuk melihat list pembaruan terbaru dari Userbot."],
